...
| **Morph Format** | 出力4(Morphs) の頂点モーフ表現。`Blend Shapes (packed)`（既定）＝ `blendshapes::2.0` 用のパック疎blendshape。`KineFX Character Blend Shapes`＝同形式＋`weight` 属性（KineFX Capture Pose 用）。`Delta Point Cloud`＝従来のデルタ点群 |
...
| 4 | **Morphs** | 頂点モーフのパック疎 blendshape（既定）/ デルタ点群＋モーフ・表示枠カタログ（Morph Format で切替） |
...
|------|------|------|
| point | `P` | 位置 |
| point | `id` (int) | 点番号（= PMX 頂点索引）。出力4の blendshape が `id` で対応付け |
| point | `N` | 法線 |
...
**Morph Format** で頂点モーフの表現を切替。group/bone/material/flip/impulse モーフと表示枠は全形式で detail カタログに格納（下表）。

**Blend Shapes (packed)（既定）**：頂点モーフごとに **1 パックプリミティブ**（prim `name`＝モーフ名）。パック内部は **影響頂点のみ（ルーズ点）**、各点 `id`＝ベース頂点索引・`P`＝**絶対モーフ位置**。
- 接続：入力0＝Mesh(0)、入力1＝本出力。`blendshapes::2.0` で *Treat Packed Primitives As Shapes* ON・*Point ID Attribute*＝`id`・*Update Channel Names From Inputs* を押し、各 blend ウェイトで適用。

**KineFX Character Blend Shapes**：Houdini ネイティブの Character Blend Shapes と同じ **影響面サブメッシュ**形式。頂点モーフごとに 1 パックプリミティブで、パック内部は **動いた頂点を含む面のみ**（point: `id`・`N`／prim: `id`、`P`＝絶対モーフ位置・未移動の隅点はベース位置）。パックに `blendshape_channel`／`blendshape_name`（＝モーフ名）と `weight`（既定0）が付き、`_3d_hidden_primitives` グループに入る。
- 接続：本出力を `Character Blend Shapes` の **Capture Pose** 入力へ。

| 区分 | 属性 | 内容 |
|------|------|------|
| prim | `name` (str) / `morph_index` (int) / `morph_panel` (int) | モーフ名・索引・表情パネル |
| prim | `blendshape_channel` / `blendshape_name` (str) / `weight` (float) | **KineFX 形式のみ**（チャンネル名・シェイプ名・ウェイト既定0、`_3d_hidden_primitives` グループ） |
| (パック内) point | `id` (int) / `P` / `N` | ベース頂点索引・絶対位置・法線 |
| (パック内) prim | `id` (int) | ベース面索引（**KineFX 形式のみ**） |

**Delta Point Cloud**：頂点/UV モーフを点群で表現（点属性 `morph_name`/`morph_index`/`morph_type`/`vtx`/`delta`/`uv_delta`）。再構成例：`morph_index` で抽出し、各点 `vtx` が指す出力0 point に `delta`×ウェイトを加算。

| 区分 | 共通カタログ属性 | 内容 |
|------|------|------|
| detail | `pmx_morphs` (dict[]) | 全モーフのカタログ。group/bone/material/flip/impulse はオフセットデータも内包。UV モーフもここに記載 |
