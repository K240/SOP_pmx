...
ボーンごとに point、親→子の**開いたポリライン**で階層を表現。
...
剛体ごとに point。`pmx_*` が正本、`jolt_*` は [`jolt`](../jolt) ソルバ向けの補助マッピングです。
...
ジョイントごとに、剛体 A 位置→B 位置を結ぶ**開いた 2 点ポリライン**。
...
頂点・UV モーフは**デルタ点群**。それ以外は detail カタログ。
...
- 出力 0 は verb cook、出力 1〜4 は `cookMySopOutput`（各出力で PMX を再解析）。
